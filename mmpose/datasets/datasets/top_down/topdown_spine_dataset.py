...
class TopDownSpineDataset(Kpt2dSviewRgbImgTopDownDataset):
    ...
    def evaluate(self, outputs, res_folder = None, metric='MRE', **kwargs):
    ...
    def _report_metric(self,
        ...
        if 'SDR_2_i2' in metrics:
             _, pck_2_i2, _ = keypoint_pck_accuracy(outputs, gts, masks,thr=20,normalize=np.ones((N, 2), dtype=np.float32))
             info_str.append(('SDR_2_i2', pck_2_i2))
        if 'SDR_2.5_i2' in metrics:
             _, pck_25_i2, _ = keypoint_pck_accuracy(outputs, gts, masks,thr=25,normalize=np.ones((N, 2), dtype=np.float32))
             info_str.append(('SDR_2.5_i2', pck_25_i2))
        if 'SDR_3_i2' in metrics:
             _, pck_3_i2, _ = keypoint_pck_accuracy(outputs, gts, masks,thr=30,normalize=np.ones((N, 2), dtype=np.float32))
             info_str.append(('SDR_3_i2', pck_3_i2))
        if 'SDR_4_i2' in metrics:
             _, pck_4_i2, _ = keypoint_pck_accuracy(outputs, gts, masks,thr=40,normalize=np.ones((N, 2), dtype=np.float32))
             info_str.append(('SDR_4_i2', pck_4_i2))
        if 'SDR_10_i2' in metrics:
             _, pck_10_i2, _ = keypoint_pck_accuracy(outputs, gts, masks,thr=100,normalize=np.ones((N, 2), dtype=np.float32))
