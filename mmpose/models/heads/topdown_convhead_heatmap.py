...
class ConvergeHead(nn.Module):
    ...
    def forward(self, x):
        hp = self.conv(x)
        hp = F.pixel_shuffle(hp, self.up_ratio) #(B, C*r*r, H，w) reshape成 (B, C, H*r，w*r)
        return hp
...
class TopdownSimple_Heatmap_ConvHead(TopdownHeatmapBaseHead):
    ...
    def get_loss(self, output, target, target_weight):
        ...
        losses['Multiresolution_loss_1'] = self.loss(output, target, target_weight)
